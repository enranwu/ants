...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while place is not beehive:
            if self.max_range >= steps_taken >= self.min_range:
                if place.bees:
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.instances < 1:
            ...
            self.is_queen = False      #Impostor queens
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        if not self.is_queen:           #For impostor queens
            self.reduce_armor(QueenAnt.armor)
        elif self.is_queen:
            ThrowerAnt.action(self, colony)
            QueenAnt_place_exit = self.place.exit
            while QueenAnt_place_exit != None:
                updated_ants = QueenAnt_place_exit.ant
                if updated_ants and updated_ants not in self.doubled_ants:
                    self.doubled_ants += [updated_ants]
                    updated_ants.damage = updated_ants.damage * 2
                if updated_ants and updated_ants.is_container and updated_ants.contained_ant and updated_ants.contained_ant not in self.doubled_ants:
                    updated_ants.contained_ant.damage = updated_ants.contained_ant.damage * 2
                    self.doubled_ants += [updated_ants.contained_ant]
                QueenAnt_place_exit = QueenAnt_place_exit.exit
    ...
    def reduce_armor(self, amount):
        ...
        if amount >= self.armor and self.is_queen:
            return bees_win()
        # END Problem 13
