    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect,QueenAnt) and insect.is_true_queen:
                return
                '''if isinstance(self.ant,BodyguardAnt) and hasattr(self.ant,'is_container') and self.ant.is_container:
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        print("DEBUG: backwards", self.going_forward)
        if self.going_forward==False and self.place.entrance!=colony.beehive:
            if self.times_scared == 2:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while location != beehive:
            if self.max_range >= count and self.min_range <= count:
                if location.bees:
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 7
        bee_locations = self.place.bees[:]
        for b in bee_locations:
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_true_queen:
            ...
            while location:
                if location.ant:
                    if isinstance(location.ant, BodyguardAnt) and location.ant.contained_ant and location.ant.contained_ant not in self.buffed:
                        location.ant.contained_ant.damage *= 2
                        self.buffed.append(location.ant.contained_ant)
                    elif isinstance(location.ant, BodyguardAnt) and location.ant.contained_ant and location.ant.contained_ant not in self.buffed:
                        location.ant.contained_ant.damage *= 2
                        location.ant.damage *= 2
                        self.buffed.append(location.ant)
                        self.buffed.append(location.ant.contained_ant)
                    elif location.ant not in self.buffed:
                        location.ant.damage *= 2
                        self.buffed.append(location.ant)
                location = location.exit
        elif not self.is_true_queen:
...
def make_scare(action, bee):
    ...
    # BEGIN Problem EC
    #turns_scared = 0
    # if it's been scared before, return the original action
    print("DEBUG: time_scared", bee.times_scared)
    if bee.times_scared >= 2:
        bee.going_forward = 'Doesnt work'
        print("DEBUG: going_forward?", bee.going_forward)
        return action

    def go_backwards(colony):
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    original = bee.action
    #keep track of when stuff is changing and being assinged prior
    current = effect(original, bee)
    dur = duration
    count = 0
    def action(colony):
        nonlocal count
